...
static bool sync_reading_to_firebase(const vital_readings_t *r) {
    if (!wifi_manager_is_connected() || strlen(s_firebase_host) == 0) {
        return false;
    }

    char url[256];
    snprintf(url, sizeof(url), "%s/READINGS/%s/latest.json", s_firebase_host, r->rfid);
    ...
    if (err == ESP_OK && (status_code >= 200 && status_code < 300)) {
        ESP_LOGI(TAG, "Successfully synced reading for [%s] to Firebase", r->rfid);
        return true;
    } else {
        ESP_LOGW(TAG, "Firebase sync failed for [%s], status: %d, err: %s", r->rfid, status_code, esp_err_to_name(err));
        return false;
    }
}

static bool fetch_user_from_firebase(const char *rfid, user_profile_t *out_user) {
    if (!wifi_manager_is_connected() || strlen(s_firebase_host) == 0 || !rfid || !out_user) {
        return false;
    }

    char url[256];
    snprintf(url, sizeof(url), "%s/USERS/%s.json", s_firebase_host, rfid);
    ...
    strncpy(out_user->rfid, rfid, sizeof(out_user->rfid) - 1);
    ...
    out_user->fingerprint_registered = (item && cJSON_IsTrue(item));

    item = cJSON_GetObjectItem(json, "fingerprint_id");
    out_user->fingerprint_id = item ? (uint8_t)item->valueint : 0;

    cJSON_Delete(json);
    return true;
}

static void cloud_sync_worker_task(void *pvParameters) {
        ...
        if (xQueueReceive(g_cloud_queue, &cmd, portMAX_DELAY) == pdTRUE) {
            switch (cmd.type) {
                ...
                case CLOUD_CMD_UPDATE_FP_STATUS: {
                    if (wifi_manager_is_connected() && strlen(s_firebase_host) > 0) {
                        char url[256];
                        snprintf(url, sizeof(url), "%s/USERS/%s.json", s_firebase_host, cmd.data.fp_status.rfid);
