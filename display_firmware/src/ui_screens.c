...
static void on_btn_save_readings(lv_event_t *e) {
    ...
    ui_show_toast("✅ Vitals & BP Sent to Cloud Database!");
}
...
static bool s_boot_transition_done = false;

static void on_boot_timer_done(lv_timer_t *t) {
    if (s_current_screen == UI_SCREEN_BOOT) {
        s_current_screen = UI_SCREEN_IDLE;
...
void ui_boot_update_status(int percent, const char *task_text, int core_ok, int wifi_ok, int cloud_ok, int sensors_ok) {
    ...
    // Smooth auto-transition to Idle screen once 100% is reached
    if (percent >= 100 && !s_boot_transition_done && s_current_screen == UI_SCREEN_BOOT) {
        s_boot_transition_done = true;
        lv_timer_t *t = lv_timer_create(on_boot_timer_done, 800, NULL);
...
static void create_boot_screen(void) {
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_BOOT], lv_color_hex(0x0f172a), 0);
    ...
    lv_obj_set_style_text_color(s_boot_task_lbl, lv_color_hex(0x38bdf8), 0);
    ...
    lv_obj_set_style_bg_color(s_boot_bar, lv_color_hex(0x1e293b), LV_PART_MAIN);
    lv_obj_set_style_bg_color(s_boot_bar, lv_color_hex(0x0ea5e9), LV_PART_INDICATOR);
    ...
    lv_obj_set_style_bg_color(diag_box, lv_color_hex(0x1e293b), 0);
    lv_obj_set_style_border_color(diag_box, lv_color_hex(0x334155), 0);
    lv_obj_set_style_border_width(diag_box, 1, 0);
    lv_obj_set_style_radius(diag_box, 10, 0);

    const char *subsystems[] = {
        "⚙️ ESP32-S3 Dual-Core (240MHz) & PSRAM",
        "📶 Wi-Fi Station (MediBot_Clinic_Net)",
        "☁️ Firebase Realtime Database SSL Sync",
        "🩺 Medical Sensors (MAX30102, AS608, Scale)"
    };
    for (int i = 0; i < 4; i++) {
        lv_obj_t *row = lv_obj_create(diag_box);
        lv_obj_set_size(row, 500, 32);
        lv_obj_set_pos(row, 0, i * 38);
        lv_obj_set_style_bg_color(row, lv_color_hex(0x0f172a), 0);
    ...
    lv_label_set_text(btn_lbl, "Enter Standby →");
    lv_obj_center(btn_lbl);
}

static void create_idle_screen(void) {
    s_screens[UI_SCREEN_IDLE] = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_IDLE], lv_color_hex(0x0f172a), 0);

    lv_obj_t *h1 = lv_label_create(s_screens[UI_SCREEN_IDLE]);
    lv_label_set_text(h1, "Welcome to MediBot");
    lv_obj_set_style_text_color(h1, lv_color_hex(0xf8fafc), 0);
    ...
    // Large Touch Card 1: Existing Patient
    lv_obj_t *btn_login = lv_button_create(s_screens[UI_SCREEN_IDLE]);
    lv_obj_set_size(btn_login, 320, 180);
    lv_obj_align(btn_login, LV_ALIGN_CENTER, -180, 40);
    lv_obj_set_style_bg_color(btn_login, lv_color_hex(0x1e293b), 0);
    lv_obj_set_style_border_color(btn_login, lv_color_hex(0x0ea5e9), 0);
    lv_obj_set_style_border_width(btn_login, 2, 0);
    lv_obj_set_style_radius(btn_login, 14, 0);
    lv_obj_add_event_cb(btn_login, on_btn_idle_existing, LV_EVENT_CLICKED, NULL);

    lv_obj_t *l1 = lv_label_create(btn_login);
    lv_label_set_text(l1, "Existing Patient\n\nLogin with Badge & Biometrics");
    lv_obj_set_style_text_align(l1, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_center(l1);

    // Large Touch Card 2: New Registration
    lv_obj_t *btn_signup = lv_button_create(s_screens[UI_SCREEN_IDLE]);
    lv_obj_set_size(btn_signup, 320, 180);
    lv_obj_align(btn_signup, LV_ALIGN_CENTER, 180, 40);
    lv_obj_set_style_bg_color(btn_signup, lv_color_hex(0x1e293b), 0);
    lv_obj_set_style_border_color(btn_signup, lv_color_hex(0x10b981), 0);
    lv_obj_set_style_border_width(btn_signup, 2, 0);
    lv_obj_set_style_radius(btn_signup, 14, 0);
    lv_obj_add_event_cb(btn_signup, on_btn_idle_new, LV_EVENT_CLICKED, NULL);

    lv_obj_t *l2 = lv_label_create(btn_signup);
    lv_label_set_text(l2, "New Registration\n\nEnroll & Assign Clinic Badge");
...
static void create_dashboard_screen(void) {
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_DASHBOARD], lv_color_hex(0x0f172a), 0);
    ...
    lv_label_set_text(s_dash_patient_name, "Sarah Jenkins");
    lv_obj_set_style_text_font(s_dash_patient_name, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(s_dash_patient_name, lv_color_hex(0xf8fafc), 0);
    ...
    lv_obj_set_style_text_color(s_dash_patient_id, lv_color_hex(0x38bdf8), 0);
    ...
    lv_obj_set_style_bg_color(bmi_box, lv_color_hex(0x1e293b), 0);
    lv_obj_set_style_border_color(bmi_box, lv_color_hex(0x334155), 0);
    ...
    lv_obj_set_style_text_color(s_dash_bmi_lbl, lv_color_hex(0xf8fafc), 0);
    ...
    lv_obj_set_style_bg_color(card_hr, lv_color_hex(0x1e293b), 0);

    lv_obj_t *l_hr_t = lv_label_create(card_hr);
    lv_label_set_text(l_hr_t, "Heart Rate");
    lv_obj_set_style_text_color(l_hr_t, lv_color_hex(0xf43f5e), 0);
    ...
    lv_obj_set_style_bg_color(card_spo2, lv_color_hex(0x1e293b), 0);

    lv_obj_t *l_spo2_t = lv_label_create(card_spo2);
    lv_label_set_text(l_spo2_t, "Blood Oxygen");
    lv_obj_set_style_text_color(l_spo2_t, lv_color_hex(0x0ea5e9), 0);
    ...
    lv_obj_set_style_bg_color(card_temp, lv_color_hex(0x1e293b), 0);

    lv_obj_t *l_temp_t = lv_label_create(card_temp);
    lv_label_set_text(l_temp_t, "Body Temp");
    lv_obj_set_style_text_color(l_temp_t, lv_color_hex(0xeab308), 0);
    ...
    lv_obj_set_style_bg_color(card_bp, lv_color_hex(0x1e293b), 0);
    ...
    lv_obj_set_style_text_color(l_bp_t, lv_color_hex(0xa78bfa), 0);
    ...
    lv_obj_set_style_bg_color(nav_bar, lv_color_hex(0x1e293b), 0);
    ...
    lv_label_set_text(l_meas, "🩺 Take Measurement");
    ...
    lv_label_set_text(l_save, "💾 Save to Cloud");
    ...
    lv_obj_set_style_bg_color(btn_prof, lv_color_hex(0x334155), 0);
    lv_obj_t *l_prof = lv_label_create(btn_prof);
    lv_label_set_text(l_prof, "History →");
...
static void create_login_screens(void) {
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_LOGIN_CARD], lv_color_hex(0x0f172a), 0);
    ...
    lv_obj_set_size(btn_sim_card, 260, 50);
    ...
    lv_label_set_text(lbl_sc, "💳 Tap RFID Badge");
    ...
    lv_obj_set_size(btn_sim_fp, 260, 50);
    ...
    lv_label_set_text(lbl_fp, "👆 Verify Biometrics");
...
static void create_signup_screens(void) {
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_SIGNUP_CARD], lv_color_hex(0x0f172a), 0);

    lv_obj_t *t1 = lv_label_create(s_screens[UI_SCREEN_SIGNUP_CARD]);
    lv_label_set_text(t1, "Patient Registration — Step 1 of 4");
    lv_obj_set_style_text_color(t1, lv_color_hex(0x38bdf8), 0);
    ...
    lv_obj_set_size(card_box1, 460, 180);
    lv_obj_align(card_box1, LV_ALIGN_CENTER, 0, 20);
    lv_obj_set_style_bg_color(card_box1, lv_color_hex(0x1e293b), 0);
    ...
    lv_label_set_text(icon_rfid, "💳\n\nRFID / NFC SENSOR ACTIVE\nPlace card firmly against the scanner pad");
    lv_obj_set_style_text_align(icon_rfid, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(icon_rfid, lv_color_hex(0xe2e8f0), 0);
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_SIGNUP_INFO], lv_color_hex(0x0f172a), 0);

    lv_obj_t *t2 = lv_label_create(s_screens[UI_SCREEN_SIGNUP_INFO]);
    lv_label_set_text(t2, "Patient Registration — Step 2 of 4");
    lv_obj_set_style_text_color(t2, lv_color_hex(0x38bdf8), 0);
    ...
    lv_obj_set_style_text_color(h2, lv_color_hex(0xf8fafc), 0);
    lv_obj_align(h2, LV_ALIGN_TOP_MID, 0, 70);

    lv_obj_t *card_box2 = lv_obj_create(s_screens[UI_SCREEN_SIGNUP_INFO]);
    lv_obj_set_size(card_box2, 500, 200);
    lv_obj_align(card_box2, LV_ALIGN_CENTER, 0, 20);
    lv_obj_set_style_bg_color(card_box2, lv_color_hex(0x1e293b), 0);
    ...
    lv_label_set_text(info_lbl,
        "Account Verified: Patient Record Found\n\n"
    ...
    lv_obj_set_style_text_color(info_lbl, lv_color_hex(0xf8fafc), 0);
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_SIGNUP_FP1], lv_color_hex(0x0f172a), 0);
    ...
    lv_obj_set_style_text_color(h3, lv_color_hex(0xf8fafc), 0);
    lv_obj_align(h3, LV_ALIGN_TOP_MID, 0, 70);

    lv_obj_t *card_box3 = lv_obj_create(s_screens[UI_SCREEN_SIGNUP_FP1]);
    lv_obj_set_size(card_box3, 460, 180);
    lv_obj_align(card_box3, LV_ALIGN_CENTER, 0, 20);
    lv_obj_set_style_bg_color(card_box3, lv_color_hex(0x1e293b), 0);
    lv_obj_set_style_border_color(card_box3, lv_color_hex(0xf59e0b), 0);
    ...
    lv_obj_set_style_bg_color(s_screens[UI_SCREEN_SIGNUP_FP2], lv_color_hex(0x0f172a), 0);

    lv_obj_t *t4 = lv_label_create(s_screens[UI_SCREEN_SIGNUP_FP2]);
    lv_label_set_text(t4, "Biometric Enrollment — Step 4 of 4");
    lv_obj_set_style_text_color(t4, lv_color_hex(0x38bdf8), 0);
    ...
    lv_obj_set_style_text_color(h4, lv_color_hex(0xf8fafc), 0);
    lv_obj_align(h4, LV_ALIGN_TOP_MID, 0, 70);

    lv_obj_t *card_box4 = lv_obj_create(s_screens[UI_SCREEN_SIGNUP_FP2]);
    lv_obj_set_size(card_box4, 460, 180);
    lv_obj_align(card_box4, LV_ALIGN_CENTER, 0, 20);
    lv_obj_set_style_bg_color(card_box4, lv_color_hex(0x1e293b), 0);
    ...
    lv_label_set_text(icon_fp2, "✅\n\nCONFIRMING BIOMETRIC TEMPLATE\nPlace the same finger to finalize registration");
    lv_obj_set_style_text_align(icon_fp2, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(icon_fp2, lv_color_hex(0x34d399), 0);
...
static void create_profile_screen(void) {
    ...
    lv_label_set_text(lbl_b, "← Vitals Dashboard");
    lv_obj_center(lbl_b);
}

void ui_init(void) {
    ESP_LOGI(TAG, "Initializing LVGL Screens & Themes");
    lvgl_port_lock(0);
    ...
    lv_obj_set_size(s_toast_obj, 420, 48);
    lv_obj_align(s_toast_obj, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_set_style_bg_color(s_toast_obj, lv_color_hex(0x1e293b), 0);
    ...
    lv_label_set_text(s_toast_lbl, "System Initialized");
    lv_obj_set_style_text_color(s_toast_lbl, lv_color_hex(0xf8fafc), 0);
